...
async def run_executor_cycle(
    ...
    while True:
        ...
        for i, op in enumerate(current_step_ops):
            ...
            if GUARDRAILS_ENABLED and guardrails.enabled:
                allowed, violation = guardrails.check_tool_call(tool_name, tool_params)
                if not allowed:
                    _get_console().print(
                        Panel(
                            f"安全围栏拦截: [{violation.rule_id}] {violation.rule_name}\n"
                            f"详情: {violation.details}\n"
                            f"工具: {tool_name}",
                            title="[bold red]围栏拦截[/bold red]",
                            style="bold red"
                        )
                    )
                    # 记录违规到图
                    graph_manager.update_node(step_id, {
                        "status": "blocked_by_guardrail",
                        "guardrail_violation": violation.__dict__ if hasattr(violation, '__dict__') else str(violation)
                    })
                    continue  # 跳过此操作的实际执行
