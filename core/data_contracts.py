...
# ============================================================================
# 安全围栏数据契约 (Guardrails Data Contracts)
# ============================================================================
...
class GuardrailViolation:
    ...
    当TestingGuardrails检测到违反红线规则的行为时，
    生成此记录并返回给调用方进行拦截或告警。

    Attributes:
        rule_id: 违规的红线规则编号，如 'R01', 'R02'
        rule_name: 红线规则名称
        severity: 违规严重程度，'block'(拦截) / 'warn'(警告)
        tool_name: 触发违规的工具名称
        details: 违规详情描述
        suggested_action: 建议的替代操作
        timestamp: 违规检测时间戳
    """
...
@dataclass
class InvalidVulnType:
    """
    无效漏洞类型记录.

    当漏洞类型被安全围栏判定为无效或不需要报告时，
    生成此记录用于统计和审计。

    Attributes:
        vuln_type: 被判定为无效的漏洞类型
        reason: 判定为无效的原因
        description: 原始漏洞描述（可选）
    """
